...
exports.getEmployeeWorkSchedule = async (req, res) => {
  ...
  try {
    ...
    const workschedule = await WorkSchedule.getWorkScheduleByEmployeeId(
      payload._id,
      month
    );
    if (!workschedule) {
      return sendResponse(
        ...
        null,
        'Work schedule not found for this employee'
      );
    }
    return sendResponse(
      res,
      ...
      schedules.map(async (employee) => {
        ...
        try {
          ...
          const mockEmployeeData =
            await Employee.getEmployeeDetails(employeeID);
          ...
          return {
            ...
            department:
              mockEmployeeData.department_name || 'Unknown Department',
            image:
              mockEmployeeData.employee_image ||
...
exports.editEmployeeWorkSchedule = async (req, res) => {
  ...
  try {
    ...
    // Update the schedule
    const updatedSchedule = await WorkSchedule.updateEmployeeWorkSchedule(
      id,
      schedule,
      month
    );
...
exports.createNewWorkSchedule = async (req, res) => {
  ...
  try {
    ...
    for (const month of months) {
      for (const day of schedule) {
        ...
        const existingSchedule =
          await WorkSchedule.getWorkScheduleByEmployeeIdMonthDay(
            employeeId,
            month,
            day.day
          );
    ...
    return sendResponse(
      ...
      httpStatus.CREATED,
      null,
      'Work schedule created successfully for all selected months.'
    );
  } catch (error) {
