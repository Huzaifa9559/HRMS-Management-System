...
exports.getEmployeeAttendanceRecords = async (req, res) => {
  ...
  try {
    ...
    const attendanceRecords = await Attendance.getAttendanceByEmployeeId(
      payload._id
...
exports.getEmployeeAttendanceRecord = async (req, res) => {
  ...
  try {
    const payload = getUser(token);
    const attendanceRecord = await Attendance.getAttendanceByEmployeeIdAndDate(
      payload._id,
      date
    ); // Fetch attendance record by Attendance ID and date
    if (!attendanceRecord) {
      return sendResponse(
        res,
        httpStatus.OK,
        null,
        'Attendance record not found for this Attendance on the specified date'
      );
    }
    return sendResponse(
      res,
...
exports.viewAttendance = async (req, res) => {
  ...
  try {
    ...
    const employeeAttendanceRecords =
      await Attendance.getAttendanceByDepartmentId(departmentId);
