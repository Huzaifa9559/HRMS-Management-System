...
WorkSchedule.getWorkScheduleByEmployeeIdMonthDay = async function (employeeId, month,day) {
    const query = `
    SELECT *
    FROM Work_Schedule 
...
WorkSchedule.updateEmployeeWorkSchedule = async function (employeeId, schedule,month) {
    const updateQuery = `
    ...
    try {
         for (const day of schedule) {
            ...
            if (updateResult === 0) {
                throw new Error(`No existing work schedule found for ${scheduleDay} in ${month}`);
            }
        }
