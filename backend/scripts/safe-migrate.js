...
function getCurrentMigrations() {
  try {
    ...
    log('⚠️  Could not get migration status (this is OK if no migrations exist yet)');
...
function loadMigrationState() {
  try {
    if (fs.existsSync(MIGRATION_STATE_FILE)) {
      const content = fs.readFileSync(MIGRATION_STATE_FILE, 'utf-8');
      return JSON.parse(content);
    }
  } catch (error) {
    log('⚠️  Could not load previous migration state');
  }
  return null;
}
...
function getPendingMigrations(currentState, previousState) {
  ...
  return currentState.filter(m => 
    m.state === 'up' && !previousNames.has(m.name)
  );
}
...
function main() {
  ...
  try {
    ...
    if (migrationsToRollback.length > 0) {
      log(`📋 Found ${migrationsToRollback.length} migration(s) that need to be rolled back:`);
      ...
      try {
        ...
        log('📋 Database has been restored to the state before migration attempt');
      } catch (rollbackError) {
        log('═══════════════════════════════════════════════════════════');
      ...
      log('ℹ️  No migrations to rollback (migration failed before applying any changes)');
