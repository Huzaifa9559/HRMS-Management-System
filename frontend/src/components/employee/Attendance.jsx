...
export default function Attendance() {
  ...
  const handleCheckIn = async () => {
    try {
      ...
      const response = await axios.get(
        `/api/employees/attendance?date=${date}`,
        {
      ...
      if (
        response.data.message === 'Attendance record retrieved successfully'
      ) {
        toast.error('You have already checked in today.'); // Show error notification
      ...
      await axios.post(
        '/api/employees/attendance/clockin',
        {},
        {
          headers: {
  ...
  const handleBreakIn = async () => {
    ...
    try {
      ...
      await axios.post(
        '/api/employees/attendance/breakin',
        { date, action },
        {
          headers: {
            Authorization: `Bearer ${token}`, // Include the token in the request headers
  ...
  const handleBreakOut = async () => {
    ...
    try {
      ...
      const date = new Date().toISOString().split('T')[0]; // Format: YYYY-MM-DD
      await axios.post(
        '/api/employees/attendance/breakout',
        { date, action },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const confirmCheckOut = async () => {
    ...
    try {
      ...
      await axios.post(
        '/api/employees/attendance/clockout',
        { date, action },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    ...
    >
      ...
      <div className="flex-grow-1" style={{ padding: '20px' }}>
        ...
        <div
          className="py-2 px-4 d-flex align-items-center justify-content-between"
          style={{
        ...
        >
          ...
          <div
            className="bg-secondary text-white d-flex flex-column align-items-center justify-content-center rounded"
            style={{
              width: '80px',
              height: '80px',
              fontSize: '1.25rem',
            ...
            <Table hover responsive className="table-borderless">
              <thead>
                <tr style={{ borderBottom: '1px solid #e9ecef' }}>
                  ...
                  <th style={{ fontWeight: 'bold', color: 'black' }}>
                    Check In
                  </th>
                  <th style={{ fontWeight: 'bold', color: 'black' }}>
                    Check Out
              ...
              <tbody>
                {filteredTimesheet
                  ...
                  .map((item, index) => {
                    ...
                    const workingTime = secondsToTime(
                      item.attendance_workingHours
                    );
                    return (
                      <tr
                        key={index}
                      ...
                      >
                        ...
                        <td>
                          {convertTo12HourFormat(item.attendance_clockIn)}
                        </td>
                        <td>
                          {convertTo12HourFormat(item.attendance_clockOut)}
                        </td>
                        <td>{`${breakTime.hours}h ${breakTime.minutes}m`}</td>{' '}
                        {/* Display break hours */}
                        <td>{`${workingTime.hours}h ${workingTime.minutes}m`}</td>{' '}
