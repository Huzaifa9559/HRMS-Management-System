    ...
    const typeParagraph = () => {
      const paragraphInterval = setInterval(() => {
        if (paragraphIndex <= paragraphText.length) {
          setTypedTextParagraph(
            (prev) => prev + paragraphText.charAt(paragraphIndex)
          );
          paragraphIndex++;
        } else {
  ...
  return (
    <div className="container-fluid vh-100">
      <div className="row h-100">
        {/* Left side */}
        <div
          className="col-lg-6 d-flex flex-column justify-content-center align-items-start text-white p-3 p-lg-4"
          style={{
            ...
            <form onSubmit={handleSubmit}>
              ...
              {submitMessage && (
                <div className="custom-alert mt-3">
                  <span
                    className={`alert-text ${submitMessage.includes('successfully') ? 'text-success' : 'text-danger'}`}
                  >
                    {submitMessage.includes('successfully') ? '✔️ ' : '❗ '}
                    {submitMessage}
                  </span>
                </div>
              )}
